...
/*
 *  GTPageController 的缓存设置，默认缓存为无限制，当收到 memoryWarning 时，会自动切换到低缓存模式 (GTPageControllerCachePolicyLoGTemory)，并在一段时间后切换到 High .
 收到多次警告后，会停留在到 GTPageControllerCachePolicyLoGTemory 不再增长
 *
 *  The Default cache policy is No Limit, when recieved memory warning, page controller will switch mode to 'LoGTemory'
 and continue to grow back after a while.
 If recieved too much times, the cache policy will stay at 'LoGTemory' and don't grow back any more.
 */
typedef NS_ENUM(NSUInteger, GTPageControllerCachePolicy){
    GTPageControllerCachePolicyNoLimit   = 0,  // No limit
    GTPageControllerCachePolicyLoGTemory = 1,  // Low Memory but may block when scroll
    GTPageControllerCachePolicyBalanced  = 3,  // Balanced ↑ and ↓
    GTPageControllerCachePolicyHigh      = 5   // High
};

@interface GTPageController : UIViewController
 ...
 */
@property (nonatomic, assign) GTMenuViewStyle menuViewStyle;
 ...
 */
@property (nonatomic, assign) GTPageControllerCachePolicy cachePolicy;
